...
const projects: Project[] = [
  { slug: 'criarsaas', title: 'Criar SaaS', url: 'https://agenciapkg.com.br/criarsaas/', meta: 'produto digital · captação', badge: 'saas launch', colors: { background: '#071018', primary: '#cdd9ff', secondary: '#18283c', accent: '#7ca7ff' } },
...
function ProjectArt({ project, index }: { project: Project; index: number }) {
  ...
  return <div className="project-art" style={{ '--project-bg': project.colors.background } as React.CSSProperties}>
    ...
    <Image src={`/projects/art-${project.slug}.webp`} alt="" fill sizes="(max-width: 719px) 88vw, (max-width: 900px) 42vw, 28vw" className={`project-art-image ${loaded ? 'is-loaded' : 'is-hidden'}`} onLoad={() => setLoaded(true)} onError={() => setLoaded(false)} />
  </div>;
}
