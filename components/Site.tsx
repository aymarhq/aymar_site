...
function Header({ dict, whatsappUrl }: { dict: Dict; whatsappUrl: string }) {
  const [open, setOpen] = useState(false); const [scrolled, setScrolled] = useState(false); const [hidden, setHidden] = useState(false); const lastScroll = useRef(0);
    const links = useMemo(() => [['#videos', dict.nav.studio], ['#trabalhos', dict.nav.work], ['#processo', dict.nav.process], ['#como', dict.nav.how], ['#oferta', dict.nav.specialist], ['#contato', dict.nav.contact]], [dict]);
...
function Dock({ dict }: { dict: Dict }) {
  ...
  return <div className={`dock ${open ? 'dock-open' : ''}`} ref={dockRef}><AnimatePresence initial={false}>{open && <motion.nav id="dock-panel" className="dock-panel" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }}><BrandLogo size="sm" className="dock-brand" label={dict.brand} /><a href="#videos" onClick={() => setOpen(false)}>{dict.nav.studio}</a><a href="#trabalhos" onClick={() => setOpen(false)}>{dict.nav.work}</a><a href="#processo" onClick={() => setOpen(false)}>{dict.nav.process}</a><a href="#como" onClick={() => setOpen(false)}>{dict.nav.how}</a><a href="#oferta" onClick={() => setOpen(false)}>{dict.nav.specialist}</a><a href="#contato" onClick={() => setOpen(false)}>{dict.nav.contact}</a><button className="dock-button dock-theme" onClick={toggleTheme}><span className="dock-sun">{theme === 'dark' ? '☼' : '◐'}</span>{theme === 'dark' ? 'light' : 'dark'}</button></motion.nav>}</AnimatePresence><div className="dock-bar"><button ref={triggerRef} className="dock-button" aria-label={dict.nav.menu} aria-expanded={open} aria-controls="dock-panel" onClick={() => setOpen(value => !value)}><span className={`dock-menu-icon ${open ? 'is-open' : ''}`}><i /><i /></span><span>{dict.nav.menu}</span></button><i className="dock-dot" aria-hidden="true" /></div></div>;
}
