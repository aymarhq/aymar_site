...
function ChatbotDemo({ dict, running }: { dict: Dict; running: boolean }) {
  ...
  return <div className={`demo-stage demo-chat ${running ? 'is-running' : ''}`}><span className="demo-label">{dict.specialist.demo}</span><div className="phone-shell"><div className="phone-top"><i className="chat-avatar">O</i><span><b>Operação · Aymar</b><small><span className="chat-online">online</span><span className="chat-thinking">digitando...</span></small></span><b>···</b></div><div className="chat-list">{messages.map((message, index) => <div className={`chat-row ${message.side} ${index % 2 === 0 ? 'has-tail' : ''}`} key={message.key} style={{ '--delay': `${index * 1.15}s` } as CSSProperties}><div className="chat-bubble">{message.text}<small>{index % 2 ? <><span>{dict.ui.sentTime.split(' ')[0]}</span> <b className="chat-check">✓✓</b></> : dict.ui.receivedTime}</small></div></div>)}<div className="chat-typing"><i /><i /><i /><span>digitando...</span></div></div><div className="chat-input"><span>Digite uma mensagem</span><b>↑</b></div></div></div>;
}
...
function RepetitiveDemo({ dict, running }: { dict: Dict; running: boolean }) {
  ...
  return <div className={`demo-stage demo-repetitive ${running ? 'is-running' : ''}`}><span className="demo-label">{dict.specialist.demo}</span><div className="records-head"><div className="record-metrics"><div><strong>{running ? count.toLocaleString(dict.lang === 'en' ? 'en-US' : 'pt-BR') : '1.248'}</strong><span>{dict.specialist.repetitive.counter}</span></div><div><strong>{saved}h</strong><span>{dict.lang === 'en' ? 'TIME SAVED' : 'TEMPO ECONOMIZADO'}</span></div></div><b className="live-status"><i />{dict.ui.live}</b></div><div className="records-origin"><span>ORIGEM</span>{origins.slice(0, 3).map(tool => <img key={tool} src={`/tools/${tool}.svg`} alt={tool} />)}</div><div className="record-list">{dict.specialist.repetitive.rows.map((row, index) => <div className={`record-row ${index === 2 ? 'record-row-unstable' : ''}`} key={row} style={{ '--row-delay': `${index * .6}s` } as CSSProperties}><img src={`/tools/${origins[index]}.svg`} alt="" aria-hidden="true" /><code>#{4821 + index}</code><span>{row}</span><em>{index === 2 ? 'reprocessando' : index === 0 ? 'processando' : 'concluído'}</em><i>✓</i></div>)}</div></div>;
}
